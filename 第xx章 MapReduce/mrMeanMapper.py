...
input_ = read_input(sys.stdin)            # 创建一个输入的数据行的列表list
input_ = [float(line) for line in input_]  # 将得到的数据转化为 float 类型
numInputs = len(input_)                   # 获取数据的个数，即输入文件的数据的行数
input_ = mat(input_)                       # 将 List 转换为矩阵
sqInput = power(input_, 2)                # 将矩阵的数据分别求 平方，即 2次方
...
print("%d\t%f\t%f" % (numInputs, mean(input_), mean(sqInput)))  # 计算均值
